...
class Tour {
    ...
    public function guardar() {
        ...
        $query = "INSERT INTO tours
                    (nombre_tour, descripcion, duracion_horas, precio_por_persona,
                     hora_encuentro, lugar_encuentro, direccion, imagen_principal,
                     rela_proveedor, activo, estado_revision, created_at)
                  VALUES
                    ('$nombre', '$desc', '$duracion', $precio,
                     $hora, '$lugar', '$direccion', '$imagen',
                     $id_proveedor, 1, 'pendiente', NOW())";
    ...
    public function actualizar() {
        ...
        $imagen_sql = $this->imagen_principal 
    ...
    public function verificar_propietario($id_tour, $id_proveedor) {
        $conexion = new Conexion();
        $res = $conexion->consultar("
            SELECT id_tour FROM tours
            WHERE id_tour = " . (int)$id_tour . "
            AND rela_proveedor = " . (int)$id_proveedor . "
    ...
    public function buscar($destino = '') {
        ...
        if ($destino) {
            $query .= " AND t.nombre_tour LIKE '%$destino%'";
        }
    ...
    public function reenviarTour($id_tour) {
        $conexion = new Conexion();
        $query = "UPDATE tours
